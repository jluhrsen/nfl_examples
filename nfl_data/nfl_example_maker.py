...
class nfl_example_maker(object):
    ...
    def __init__(self, home_team, away_team, season, week):
        ...
        self.home_team_game_list = self.home_team_history.\
            get_games_up_to_specific_week(home_team, season, week - 1)

        self.home_team_average_stats = self.home_team_history.\
            calculate_averages(home_team, self.home_team_game_list)
        self.home_team_average_stats = self.append_string_to_dictionary_keys \
            ('HOME', self.home_team_average_stats.stat_dict)

        self.home_team_dvoa_stats = self.dvoa_stat_handler.\
            get_dvoa_stat_dict(self.dvoa_home_team, season, week)
        self.home_team_dvoa_stats = self.append_string_to_dictionary_keys \
            ('HOME', self.home_team_dvoa_stats)

        # reminder, we don't want to include self.week's games, just "up to" that week.
        self.away_team_game_list = self.away_team_history.\
            get_games_up_to_specific_week(away_team, season, week - 1)

        self.away_team_average_stats = self.away_team_history.\
            calculate_averages(away_team, self.away_team_game_list)
        self.away_team_average_stats = self.append_string_to_dictionary_keys \
            ('AWAY', self.away_team_average_stats.stat_dict)

        self.away_team_dvoa_stats = self.dvoa_stat_handler.\
            get_dvoa_stat_dict(self.dvoa_away_team, season, week)
        self.away_team_dvoa_stats = self.append_string_to_dictionary_keys \
            ('AWAY', self.away_team_dvoa_stats)
    ...
    def append_string_to_dictionary_keys(self, string, dict):
        '''
        ...
        :param dict: dictionary to rework
        :return: new dictionary with modified key
        '''
        new_dict = {}
        for k in dict.keys():
            new_dict[string + k] = dict[k]
