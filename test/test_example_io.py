...
class TestExampleIO(unittest.TestCase):
    ...
    def test_write_dict_as_csv(self):
        ...
        sample_dict = {sample_dict_key_order[0]: "jamo", \
                       sample_dict_key_order[1]:  "100"}
    ...
    def test_one_off_for_specific_game(self):
        ...
        self.writer.write(example.example_data_dict, key_order, \
                          self.csv_file, 'a')

        self.assertDictEqual(example.example_data_dict, \
                self.writer.read(key_order, self.csv_file)[1])
    ...
    def test_write_multiple_examples_as_csv(self):
        ...
        examples.append(nfl_example_maker('DET', 'KC', 2007, 16))
        ...
        for i in range(len(examples)):
            self.writer.write(examples[i].example_data_dict, key_order, \
                         self.csv_file, 'a')
        ...
        for i in range(len(examples)):
            ...
            self.assertDictEqual(examples[i].example_data_dict, \
                                            dict_as_read_from_csv_file[i + 1])
